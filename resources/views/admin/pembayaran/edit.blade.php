...
<div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-500 text-sm">
            Perbarui data pembayaran pendaftaran.
        </p>
    </div>
    ...
    <form action="{{ route('pembayaran.update', $pembayaran) }}" method="POST" enctype="multipart/form-data">
            ...
            <select name="pendaftaran_id"
                ...
                @foreach($pendaftarans as $pendaftaran)
                    <option value="{{ $pendaftaran->id }}"
                        {{ old('pendaftaran_id', $pembayaran->pendaftaran_id) == $pendaftaran->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="total_bayar" value="{{ old('total_bayar', $pembayaran->total_bayar) }}"
                   class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-200">
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tanggal_bayar"
                   value="{{ old('tanggal_bayar', \Carbon\Carbon::parse($pembayaran->tanggal_bayar)->format('Y-m-d')) }}"
                   class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-200">
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="file" name="bukti_pembayaran" accept="image/*,.pdf"
                   class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-200">
            <p class="text-gray-400 text-xs mt-1">Kosongkan jika tidak ingin mengganti file.</p>
        </div>
