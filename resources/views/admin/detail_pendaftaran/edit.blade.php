...
<div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">
            Edit Detail Pendaftaran
        </h1>
        <p class="text-gray-500 text-sm">
            Perbarui data detail pendaftaran praktikum.
        </p>
    </div>
    ...
    <form action="{{ route('detail_pendaftaran.update', $detailPendaftaran) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <select name="pendaftaran_id"
                ...
                @foreach($pendaftarans as $pendaftaran)
                    <option value="{{ $pendaftaran->id }}"
                        {{ old('pendaftaran_id', $detailPendaftaran->pendaftaran_id) == $pendaftaran->id ? 'selected' : '' }}>
            ...
            <select name="kelas_praktikum_id"
                ...
                @foreach($kelasPraktikums as $kelasPraktikum)
                    <option value="{{ $kelasPraktikum->id }}"
                        {{ old('kelas_praktikum_id', $detailPendaftaran->kelas_praktikum_id) == $kelasPraktikum->id ? 'selected' : '' }}>
        ...
        <div class="mb-6">
            ...
            <input type="number" step="0.01" name="harga" value="{{ old('harga', $detailPendaftaran->harga) }}"
                   class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-200">
        </div>
